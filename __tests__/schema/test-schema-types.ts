...
// Test data to validate our types
...
// Test function to validate the relationship
function validateProgramRequirements(program: Program, requirements: Requirements): boolean {
  // One-to-one relationship validation
  if (requirements.program_id !== program.id) {
    console.error('Requirements program_id does not match program id');
    return false;
  }
  
  // Validate data types
  if (requirements.ielts_score && (requirements.ielts_score < 0 || requirements.ielts_score > 9)) {
    console.error('IELTS score should be between 0 and 9');
    return false;
  }
  
  if (requirements.toefl_score && (requirements.toefl_score < 0 || requirements.toefl_score > 120)) {
    console.error('TOEFL score should be between 0 and 120');
    return false;
  }
  
  if (requirements.min_gpa && (requirements.min_gpa < 0 || requirements.min_gpa > 4)) {
    console.error('GPA should be between 0 and 4');
    return false;
  }
  
  console.log('✅ Program and Requirements validation passed');
  return true;
}
...
console.log('Sample Requirements:', JSON.stringify(sampleRequirements, null, 2));

const isValid = validateProgramRequirements(sampleProgram, sampleRequirements);
